        ...
        <section class="col-12">
            <section class="main-body-container">
                ...
                <section class="d-flex justify-content-between align-content-center mt-4 mb-3 border-bottom">
                    <a href="{{route('admin.user.customer.create')}}" class="btn btn-info stn-sm mb-4">ایجاد
                                                                                                       مشتری
                                                                                                       جدید</a>
                        ...
                        @foreach($users as $key => $user)
                            <tr>
                                ...
                                <td class="width-22-rem text-left">
                                    <a href="{{ route('admin.user.customer.edit',$user->id) }}" class="btn
                                    btn-primary btn-sm">
                                        <i class="fa fa-edit"></i>
                                        ویرایش</a>

                                    <form class="d-inline" action="{{route('admin.user.customer.destroy',
                                    $user->id)}}"
                                          method="post">
                                        @csrf
                                        {{method_field('delete')}}
                ...
                success: function (response) {
                    if (response.status) {
                        if (response.checked) {
                            ...
                            successToast('کاربر مشتری با موفقیت فعال شد')
                        } else {
